...
namespace kx {
    ...
    void InfoBuilder::RenderGadgetDetails(
        ...
        // Show gadget name (SID:XXXXX from species lookup, or OBJ:agentId fallback)
        if (gadget->name[0] != '\0') {
            auto res = std::format_to_n(buf, std::size(buf), "ID: {}", gadget->name);
            DrawLine(drawList, cursor, style, std::string_view(buf, res.size));
        }
        else {
            auto res = std::format_to_n(buf, std::size(buf), "OBJ: {}", gadget->agentId);
            DrawLine(drawList, cursor, style, std::string_view(buf, res.size));
        }

        // GadgetType::None = 26 means no specific classification -- skip to avoid showing "Type: ID: 26"
        if (settings.showDetailGadgetType && gadget->type != Game::GadgetType::None) {
            const char* gadgetTypeName = Formatting::GetGadgetTypeName(gadget->type);
            if (gadgetTypeName) {
                auto res = std::format_to_n(buf, std::size(buf), "Type: {}", gadgetTypeName);
                DrawLine(drawList, cursor, style, std::string_view(buf, res.size));
            }
        }

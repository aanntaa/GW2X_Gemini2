"""V66 persistent commander-record discovery controller.

No MumbleLink, coordinates, record IDs, or nearby entities are used. The DLL
hooks the proven remote-record writer by AOB and snapshots its record object
during a controlled tag lifecycle.
"""
...
BUILD = "GW2X-COMMANDER-RECORD-PROBE-V66.0"
IPC_MAGIC = 0x36505243
IPC_VERSION = 1
...
def main():
    ...
    if os.name != "nt":
        raise SystemExit("V66 must run on Windows.")

    print(BUILD)
    pid = choose_pid(args.pid)
    tag = rf"Local\GW2X_COMMANDER_RECORD_PROBE_V66_{pid}"
    try:
        mm = mmap.mmap(-1, IPC.size, tagname=tag, access=mmap.ACCESS_WRITE)
    except OSError as error:
        raise SystemExit(
            "V66 mapping unavailable. Build/load KX-Vision-v66 into the observer first."
        ) from error

    output = Path("commander_record_probe_v66") / dt.datetime.now().strftime("%Y%m%d-%H%M%S")
    output.mkdir(parents=True, exist_ok=True)
    logger = logging.getLogger("v66")
    logger.setLevel(logging.INFO)
    formatter = logging.Formatter("%(asctime)s %(message)s", "%H:%M:%S")
    for handler in (
        logging.StreamHandler(sys.stdout),
        logging.FileHandler(output / "commander_record_probe_v66.log", encoding="utf-8"),
    ):
    ...
    try:
        ...
        if state["magic"] != IPC_MAGIC or state["version"] != IPC_VERSION:
            raise SystemExit("V66 DLL/controller ABI mismatch.")
        if state["pid"] != pid:
            raise SystemExit("V66 mapping belongs to another process.")
        ...
        (output / "commander_record_probe_v66.json").write_text(
